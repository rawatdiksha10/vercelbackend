      ...
      const jpLevelUserIds = jpLevelUserResult.flat().map(result => result.userid);
        ...
        if (jpLevelUserIds.length>0 && (expOver || expUnder)) {
          ...
          if(specUserIds.length > 0 && certificate) {
            ...
            if (certificateUserIds.length> 0 && language.length> 0) {
              ...
              const UserTagResult = await UserTag.find({$and: [
                ...
                if (userTagUserIds.length>0 && searchKey) {
            ...
            }else if (certificateUserIds.length> 0 && searchKey) {
          ...
          } else if (specExpUserIds.length > 0 && language.length> 0) {
          ...
          } else if (specExpUserIds.length > 0 && searchKey) {
        ...
        } else if (jpLevelUserIds.length>0 && language.length> 0) {
          const UserTagResult = await UserTag.find({$and: [
            {userid: { $in: jpLevelUserIds } },
            { lbltgid: { $in: labelIds } }
          ]});
        ...
        }else if (jpLevelUserIds.length>0 && searchKey) {
...
async function searchByKeyword(searchKeyword: any): Promise<any> {
  try {
    ...
    const results = await Promise.all([
      ...
      User.aggregate([
        { $match: {$and: [{name: { $regex: searchKey? searchKey : "", $options: "i" } },{roleflg: 2}, {delflg: 0}] }},
        { $group: {
            _id: "$name",
            userIds: { $push: "$userid" }
          }
